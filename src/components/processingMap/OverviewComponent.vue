...
export default {
  ...
  watch: {
    ...
    userData () {
      ...
      this.userData.forEach(log => {
        log.nodes.forEach(node => {
          ...
          if (node.nodeType === 'End') {
            // 经历到结局的人数
            if (this.endOverviewOption.series.data.find(x => x.name === node.nodeType + node.nodeIndex) !==
                                undefined) {
              this.endOverviewOption.series.data.find(
                x => x.name === node.nodeType + node.nodeIndex).value++
            } else {
            ...
            // 到结局的耗时
            if (this.costTimeOption.xAxis[0].data.find(x => x === node.nodeType + node.nodeIndex) !==
                                undefined) {
              this.costTimeOption.series[0].data[
                this.costTimeOption.xAxis[0].data.findIndex(
                  x => x === node.nodeType + node.nodeIndex)] += node.costTime
            } else {
      ...
      // 热力图重新设置最大范围
      this.dateHeatMapOption.visualMap.max = this.GetDatvminaxNum()
      // 平均时长求值
      ...
      // 刷新绘制
      this.$refs.endPivminap.setOption(this.endOverviewOption)
  ...
  methods: {
    ...
    GetNodeCategoryNum () {
      this.graph.data.nodes.forEach(node => {
        if (this.nodeOverviewOption.series.data.find(x => x.name === nodeType.get(node.nodeType)) !==
                        undefined) {
      ...
      this.$refs.nodePivminap.setOption(this.nodeOverviewOption)
    },
  },
}
</script>
    ...
    <el-row :gutter="20">
        <el-col>
            <el-card>
                ...
                <vchart style="height: 10vmin;width: auto" :option="dateHeatMapOption" ref="dateHeatMapChart"
    ...
    <el-row :gutter="20">
        <el-col :span="12">
            <el-card>
                ...
                <vchart style="height: 10vmin;width: auto" :option="nodeOverviewOption" ref="nodePivminap"
                        :autoresize="true"></vchart>
            </el-card>
        </el-col>
        <el-col :span="12">
            <el-card>
                ...
                <vchart style="height: 10vmin;width: auto" :option="endOverviewOption" ref="endPivminap"
                        :autoresize="true"></vchart>
            </el-card>
        </el-col>
    </el-row>
    <el-row :gutter="20">
        <el-col>
            <el-card>
                ...
                <vchart style="height: 10vmin;width: auto" :option="costTimeOption" ref="endCostMap"
                        :autoresize="true"></vchart>
            </el-card>
        </el-col>
    </el-row>
